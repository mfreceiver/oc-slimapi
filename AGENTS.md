...
| **opencode 源码（在 ocdroid 内）** | `/home/mar/personal_projects/ocdroid/opencode-src/v1.17.20` | 上游 server 源码快照，供对照 legacy 行为 |

**opencode 源码目录（相对 ocdroid 根）**：`opencode-src/v1.17.20/`  
当前对齐版本：**opencode v1.17.20**（完整 monorepo 树；非部分抽取）。

### 上游对照常用路径（相对 `opencode-src/v1.17.20/`）
