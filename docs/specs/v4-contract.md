 ...
 "capabilities": {
   "4": {
     ...
     "qpImmediateFull": true,     # B3b 起已广告（同批落地；语义由 design-v4-qp-payload.md 结论冻结）
      ...
      "tokenFrameSeq": true        # 4.12.0 起已广告（同批落地）——token 流业务帧 payload `seq` 原子序号（§7.7）
   }}}
```
...
- **4.12.0 静态加性键（修订六同批落地）**：`tokenFrameSeq`（token 流业务帧 payload `seq` 原子序号，§7.7）——静态键表示**二进制支持**；本连接是否实际激活以 token 流 `slimapi.meta` 首帧是否携带该能力为准（重放日志未接线的栈——仅测试/极简形态——meta 不含该键且帧无 `seq`，生产 v4 形态恒接线）。键缺席 = 旧 sidecar 帧 `'seq' in data` 恒 false，客户端按无序号帧消费。
...
- **resync 帧 reason 值域（v4 冻结，加性扩展）**：`epoch_changed` | `replay_expired` | `replay_gap` | `reconnect_no_replay`（既有）；token 流 tombstone（消息已撤销）在 replay 时**照常消耗其 seq 并以 `message.removed` 轻量撤销帧回放**（既有帧形 `tokenstream/frames.py:137-151` = `event: message.removed` + `{sessionID, messageID}`；保留 `id:`，维持 ID 序列无空洞）。**修订六注记（4.12.0）**：本值域约束的是**控制面 resync 帧**（无 `id:`、不重放）；token 流自 4.12.0 起另有一类**可重放业务 resync**（`token_memory_limit`，带 `id:` + payload `seq` + 写重放日志，§7.7），两类集合显式不相交、服务端结构性禁止互串（实现 `tokenstream/fanout.py` 分离集合）——控制面值域不受影响。
...
- **flush-before-asked 因果序（修订六，4.12.0 [冻结]）**：q/p 直推的 asked 族（`question.asked` / `question.v2.asked`）**立即直推前**，sidecar 对该 sid 做 targeted flush——将该 sid 的 pending digest entry（含 `messagesRevision` 若挂起，§7.5）立即成帧发出。保证：**同一 SSE 流上订阅者先收 digest（携带 part/message 级 revision）再收 asked 帧**——asked 携带的阻塞快照不落后于已推送的 digest 水位。**GET 冻结边界（终门控 rev-2 裁定）**：asked 后客户端 GET `/messages` 仅保证取到 **asked 时刻已持久化**的关联消息及其依赖的 text / tool-input / running part 状态；**不**保证回答过程中才产生的 tool completion / output（它们经后续 revision bump → digest → 常规对账到达）。无 pending entry 时无操作（零额外帧）。`permission.*` 族与 asked 的 replied/rejected 不触发（无同等因果需求）。
...
- **`GET /slimapi/sessions/status`（只读 status 投影；Q4 转录）**：透传上游 `GET /session/status`（`Record<SessionID, {type:"busy"|"idle"|"retry"}>`）+ sidecar merge 每条目 flat 顶层 `turnIncarnation`/`turn`（源自 `TurnRegistry.snapshot`，与 digest SSE turn 字段同源——§7.5 digest 字段集条；未观测 sid → `(inc,0)`；turn_registry 未装配**或持久化未确认（4.12.1，§7.5 配对缺失触发面）**时两字段配对缺省）。端点只读、不写、不缓存——同内存只读投影，不引入新状态机。`directory` 可选（不传 → 200 全局 map + 不转发 directory；传 → normalize+透传——上游 handler 零参数，directory 对上游是 no-op，恒返全量 map）。上游非 dict body → 503 `upstream_unavailable`；上游 4xx → 502 `upstream_http_N`；5xx/网络 → 503。v4 差异注记见 §10 既有载（零 v4 分叉；directory 消费 = §5.1 基线）。
...
> **修订八 [2026-08-24]**（owner 批准的正式契约修订；随 4.14.0 minor 发布）：**范围** = §10.2 第 4 条 thin placeholder 语义单点——busy skeleton 的 thin placeholder part **去显示文案**。**修订后语义（冻结）**：thin placeholder part 保留 `text` 字段但值恒为**空串 `""`**（part 无有效文本内容）；**唯一识别依据 = part ID 前缀 `thin_placeholder_`**（双仓镜像稳定机器标记——sidecar `routes/messages/_full_merge.py` 识别该前缀，webui codecs 测试亦引用）；客户端**不得依赖** part 文本内容；busy 展示态（如「思考中」）由**客户端自行渲染**，服务端不替客户端决定展示文案。**owner 例外声明（wire 版本不变的破坏性 wire 形状变更）**：`text` 值由显示文案（`"[内容已折叠，点开查看]"`）变为空串属破坏性 wire 形状变更，而 wire 版本维持 4——经 owner 批准随 4.14.0 **minor** 发布（依据 = 发版规约 owner 例外规则（`AGENTS.md` 版本双轨/`docs/release.md:37-38/54-55`），4.9.0 修订四先例；动因 = webui 用户反馈该文案泄漏至客户端被原样渲染，「点开查看」属 UI 交互语义不应由服务端决定；webui 已有保底渲染方案——识别 ID 前缀 + 空内容显示「思考中」）。客户端必改点：**依赖 placeholder `text` 显示文案的消费方需改**（按 ID 前缀识别 + 自渲染展示态）；webui 已兼容零必改，ocdroid 不渲染该文案零必改。例外同步记入 CHANGELOG [4.14.0] Changed。**修订边界**：无新 readiness feature ID（§3.3 全集不变——本修订不经门控、直接修订 §10.2 基线语义）；无新错误码；ID 前缀 `thin_placeholder_` **不变**；注入时机（无任何可渲染 part 时注入）与 merged 合并逻辑（placeholder-first 优先级，第 5 条）、SSE 帧结构、`omitted:["parts"]`/`hasFull:true` 键**零改动**；表示域 validator bump（skeleton REP_VERSION `skeleton-v2` → `v3`）——同输入下新旧 v4 validator 必然不同，升级后旧 v4 ETag 全部自然失效重拉（4.9.0 后又一次全量轮换，预期内一次性重拉）；`contentFingerprint` 无 `vN` 改动（placeholder text 经投影后的表示自然进入指纹）。
...
4. **可渲染性**：skeleton 模式 `text:null` 但携带 `expandRefs` 的 part **计为可渲染**（part 骨架 + 展开入口，非整页 placeholder）；消息级省略（diffs）不参与可渲染判定；`thin_placeholder` 注入时机不变（无任何可渲染 part 时注入）。**[修订八]** thin placeholder part **无有效文本内容**（`text` 恒为空串 `""`）——以 ID 前缀 `thin_placeholder_` 为**唯一**机器识别依据，客户端**不得依赖** part 文本；busy 展示态（如「思考中」）由客户端自行渲染（修订头：owner 例外，随 4.14.0 minor）。
