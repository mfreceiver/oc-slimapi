...
> | **2026-07-22** | **1（additive，未 bump）** | **I** | **session.created→父 digest childrenVersion（X-main 失效）**：hub 新增 `session.created` 处理——提取子 `info.parentID`→`children_cache.invalidate(parentID)`（bump generation + 驱逐父 cache）+ 触发**父** digest 带 `childrenVersion=generation_of(parentID)`；`DigestFields` 加 `children_version` 字段；`HubRegistry.set_children_cache` 接线（仿 `set_transforms`）。客户端 digest 收到更大 childrenVersion→重拉 `GET /slimapi/sessions/{sid}/children`（缓存已失效→fresh fetch）。**Additive，未 bump** `X-Slimapi-Version`。落地对照 §3（digest childrenVersion）/ §16（invalidate 语义）。受影响 CLIENT_CHANGES：session.digest childrenVersion 消费。 | §3 / §16 |
> | **2026-07-27** | **1（additive，未 bump）** | **K** | **Stage B v0.4 — partEventRevision→messageEventSeq + /full 304 fingerprint + removal**：digest 加 `contentRevisions?`（messageEventSeq 严格单调替代 per-part max，CRITICAL 2）；`/full/{mid}?known.maxPartId=&known.partCount=&known.messageEventSeq=` 304 短路（CRITICAL 1）；`X-Message-Event-Seq` 响应头；`message.part.removed`/`message.removed` hub 路由（MAJOR 5）；truncated 帧顺序修复（MAJOR 4）；reconnect pending 泄漏修复（MAJOR 3）；`_part_state` LRU cap 500（MAJOR 5）。**Additive，未 bump**。delta spec：`docs/ocmar/specs/2026-07-27-stage-b-impl-spec-v0.4-delta.md`。受影响 CLIENT_CHANGES：Stage B v0.4 段。 | §2 / §3 / §3.S / §5 |
> | **2026-07-27** | **1（additive，未 bump）** | **L** | **Stage B v0.5 — messageEventSeq per-session 全局单调 + removal 自愈 + header 稳定性**：`messageEventSeq` 改 per-session 全局单调序号（CRITICAL 1 修复）；`message.part.removed` of unknown message 也产生 digest（MAJOR 2 修复）；`X-Message-Event-Seq` body 前后稳定性（MAJOR 3 修复）。**Additive，未 bump**。delta spec：`docs/ocmar/specs/2026-07-27-stage-b-impl-spec-v0.5-delta.md`。受影响 CLIENT_CHANGES：Stage B v0.5 段。 | §3.S / §5 |
> | **2026-07-27** | **1（additive，未 bump）** | **M** | **Stage B v0.6 — per_part_revision 独立递增 + message.removed tombstone 重放队列**：TokenStreamHub 独立维护 per-part revision（新 MAJOR 修复）；`message.removed` token stream tombstone 帧 + 重放队列 cap 1000/TTL 24h（MAJOR 4 方案 C）；握手时序：server.connected→tombstone 重放→flush_sid→snapshot→fanout。**Additive，未 bump**。delta spec：`docs/ocmar/specs/2026-07-27-stage-b-impl-spec-v0.6-delta.md`。受影响 CLIENT_CHANGES：Stage B v0.6 段（`message.removed` 帧消费 + 握手期处理）。 | §3 / §3.S / §3.x |
...
| GET | `/slimapi/messages/{sid}/full/{mid}` | A | 🔒 (Stage B v0.4 R2 304 + X-Message-Event-Seq 🆕) | 单条全文（mode=full，展开某条）；`?known.maxPartId=&known.partCount=&known.messageEventSeq=` 三者齐全+缓存命中+三元组一致→304；200 加 `X-Message-Event-Seq:<int>`（无缓存=0） |
...
## §3 SSE 契约（简化版，A1-A3 落定）🔒 + archived 🆕 + G1 🆕 + reconfigured 🆕 + Stage B v0.4 contentRevisions 🆕
- 上游：**一条** `/global/event`（进程级 GlobalBus，全实例跨目录，每事件自带 `directory`）。
- 帧：
  - `session.digest`（debounce 250ms/session，仅发有变化的字段）：`{sessionID, directory, status?, messageID?, updatedAt?, archived?, deleted?, lastError?, childrenVersion?, contentRevisions?}`。
    - **`contentRevisions`（Stage B v0.4 🆕，CRITICAL 2 修复）**：optional map `{messageID → messageEventSeq_int}`。当 debounce 窗口内有该 session 的 `message.part.updated` / `message.part.removed` 事件触及某 message 时，该 message 进 map，value = 该 message 的 messageEventSeq（message-level 单调事件序号；首次触及=1，每后续 `message.part.updated` / `message.part.removed` +=1）。客户端用 strict `>` 检测 message 内容变更触发 R1/R2 恢复。**单调性**：不同于 v1 的 `max(per-part revision)`（多 part 下新 part 可能把 max 拉低），messageEventSeq 在该 message 任意 part 的任意 part 事件上 +1，**严格单调**——多 part 下加一个新 part 也会让 seq 前进。空 map / 无 part 事件 → 字段省略（向后兼容：无 part 事件的 digest wire 形态不变）。`message.removed` 删除 message 时从 pending map 移除该 message 条目（digest 不带已删 message 的 contentRevision）。进程重启 / reconnect → `_part_state` 清空 + pending `content_revisions` 清空 → 后续 digest 不带 contentRevisions 直至新事件抵达（client 视为 0 / 不信任 → R1）。
...
- **Stage B v0.4 hub 事件路由**（不再"全丢"）：`message.part.updated`（嵌套 `properties.part`）→ 维护 `_part_state[psid][pmid]`（per-part revision 为 GlobalHub 内部冗余 counter，不再用于 token 帧 dedup；token 帧 dedup 使用 per-frame revision，见 §3.S v0.6 + message-level messageEventSeq 供 digest/R2/头）+ 写 debounce 窗口的 `contentRevisions` + 通知 token hub（不传 part_revision，token hub 独立维护 per-part revision，见 §3.S v0.6）；`message.part.removed`（flat props）→ parts.pop + seq+1 + digest 更新；`message.removed`（flat props）→ `_part_state` 删 message + 清该 message 在 pending contentRevisions。`message.part.delta`（token stream 逐 token）**不**触碰 `_part_state` / digest（client 实时收 delta，无需 fingerprint 信号）。
...
### §3.S Stage B v0.5 单条消息 fingerprint（R2 304 + X-Message-Event-Seq）🆕

> v0.4 → v0.5（2026-07-27 双方冻结）：`messageEventSeq` 改 per-session 全局单调序号（CRITICAL 1 修复）；`message.part.removed` of unknown message 也产生 digest（MAJOR 2 修复）；`X-Message-Event-Seq` body 前后稳定性（MAJOR 3 修复）。MAJOR 4（message.removed wire tombstone）待 ocdroid council。

- **缓存**：`GlobalHub._part_state = {sessionID: {messageID: {"parts": {partID: per_part_rev}, "seq": messageEventSeq}}}` + `GlobalHub._session_event_seq = {sessionID: int}`（per-session 全局单调序号）。`per_part_rev`（新 part=0，该 part 的 `message.part.updated` +1）**不**再用于 token 帧 dedup（token 帧 dedup 使用 per-frame revision，见下）；`messageEventSeq`（**v0.5 §K**：per-session 全局 counter 当前值——该 session 任意 message 的任意 `message.part.updated` / `message.part.removed` 事件都让全局 counter +1；该 message 的 seq = 当前全局值快照）供 digest `contentRevisions` / R2 304 / `X-Message-Event-Seq` 头。两者**独立递增**。LRU cap 500 message/session（超限淘汰最旧 message——**v0.5 §K**：淘汰不重置全局 counter，重触及 message seq = 下一个全局值，严格 `>` 旧值）。
- **`GET /slimapi/messages/{sid}/full/{mid}?known.maxPartId=&known.partCount=&known.messageEventSeq=`**（**Stage B v0.4 R2**，CRITICAL 1 修复）：三者齐全 + 缓存命中 + `(maxPartId, partCount, messageEventSeq)` 三元组全一致 → `304 Not Modified`（空 body，`Cache-Control: no-store`，**不发** `X-Message-Event-Seq`）；任一不一致/部分参数/无缓存 → 正常 full body 200。**仅 `maxPartId+partCount` 不足以 304**——同 part 文本追加 / part 互换会让该二元组碰巧相同但内容已变；`messageEventSeq` 单调序号捕获所有 part 事件。
- **`X-Message-Event-Seq: <int>`**（**Stage B v0.4 §D + v0.5 §M 稳定性**）：`/full/{mid}` 200 响应必带，值=该 message 的 messageEventSeq；无缓存（冷启动/reconnect/session.deleted/message.removed）→ `0`（client 视为"无信息"→ R1）。304 路径不发该头（无 body，fingerprint 已对齐）。**v0.5 §M**：handler body 前 `seq_pre`、body 后（transform 完成、Response 构造前）`seq_post` 两次取样；`seq_pre == seq_post` → 发 `seq_post`（可信对应 body）；不一致（body 期间 part event 到达）→ 发 `0`（不可信 → client R1）。
- **`message.part.removed` 自愈（v0.5 §L MAJOR 2 修复）**：即使 `_part_state[sid][mid]` 不存在（被 cap 淘汰/未知），也 bump 全局 seq + 写 digest `content_revisions[mid] = seq`（client strict `>` 检测 → R1 → `/full?known=` 无缓存 fingerprint → 200 自愈）。msg_entry 存在时仍 pop partID + bump seq。
- **per_part_revision 独立递增 + per-frame revision（v0.6 §Q Option B）**：`TokenStreamHub._part_revisions[key]` 由 token hub 自己维护，**不依赖** `GlobalHub.publish` 转发。此前 GlobalHub 的 `_part_state` 在 LRU cap 淘汰 message entry 后，同一 PartKey 再 `message.part.updated` 会把 per_part_rev 当成 0 → 覆盖 token hub 中更高的 revision → client strict `>` 漏帧。v0.6 修复后，每个具有独立 delivery 语义的 token 帧（`message.part.snapshot{done:false}` / `message.part.delta` / `message.part.snapshot{done:true}` / `message.part.snapshot{truncated:true}`）在发射时获得**唯一递增**的 `partEventRevision`（per-frame revision）。客户端按 strict `>` 去重——只接受 revision 大于上一次已见帧的帧；由于每帧 revision 不同，不会误丢合法帧。part 退役（`drop_part` / truncate / evict / TTL / session cleanup）时清 `_part_revisions[key]`，后续新帧 revision 从 0 重新开始。**wire `partEventRevision` 字段名不变**。
- **生命周期**：`resync_all()` / reconnect → 清 `_part_state` + `_session_event_seq`（**v0.5 §K**）+ 清所有 pending `content_revisions`；`session.deleted` → pop 该 sid 的 `_part_state` + `_session_event_seq`（**v0.5 §K**）；`message.removed` → pop 该 message 的 `_part_state` + 清该 message 在 pending `content_revisions` + 路由到 token hub 的 `on_message_removed`（**v0.6 §P MAJOR 4 方案 C**：tombstone 帧 + 重放队列）。
...
# 7) message.removed tombstone（Stage B v0.6 §P，MAJOR 4 方案 C）
event: message.removed
data: {"sessionID":"…","messageID":"…"}
```

- **不发 SSE `id:` 字段**；`Last-Event-ID` 仅触发首帧 `resync{reconnect_no_replay,sessionID}`，**值忽略**。
- **message.removed 重放队列（Stage B v0.6 §P，MAJOR 4 方案 C）**：服务端维护全局 FIFO 重放队列（cap 1000，TTL 24h），记录每个上游 `message.removed` 事件。`attach_subscriber` 握手时序为：`server.connected` → **该 session 未过期 tombstones 按时间重放** → `flush_sid` → live snapshot → 入 fanout。客户端在握手期收到 `message.removed` 帧后应清该 message 本地 streamOwned 状态（若该 message 正在 stream 中）并视为完成（后续 `/since` 拉权威全文时该 message 已不存在）。重放队列不受 `resync_all` / `on_upstream_reconnect` 影响（专为 reconnect 服务，仍受 cap/TTL 限制）。
...
- **阈值化 skeleton（加性，wire 版本仍 1；默认常开，单用户产品无 opt-in）**：tool/patch 的 `state.output`/`state.error` 不再无条件剥离——按 **JSON 字节**（`orjson.dumps` 序列化长度 = 上线字节）阈值化：per-field ≤ 4 KiB **且** 该 message 累计内联 ≤ 16 KiB → **原样内联**进 thin state；超任一阈值 → **整字段 omit**（**绝不半截断**）+ `omitted`，可经 `/full` 取回完整值。`state.structured/result/raw/attachments` **始终 omit**。**`hasFull` 仅当该 part 仍有 omitted 字段才置 `true`**（全内联且无其他删字段 → 不伪标；只表示"还有可经 full 取回的字段"，绝不表示"当前内容不可见"）。env 调参 `OC_SLIMAPI_SKELETON_INLINE_OUTPUT_MAX_BYTES`/`_MAX_MESSAGE_BYTES`（不改契约）；外层仍受 `max_response_bytes` 约束。`/slimapi/health` 加性 `features.thresholdedSkeleton=true` + `skeletonInlineOutputMaxBytes=4096`（**仅诊断**，行为不依赖客户端识别，不 bump `X-Slimapi-Version`）。字段表见 `design-v2.md` §2。
...
- **内存预算 = Option B（拆 4+4，不双计）+ handshake buffer**：
  ...
  - `TOKEN_HANDSHAKE_BUFFER_BYTES=8MiB/sub`：每个新 subscriber 握手期间（`attach_subscriber` snapshot 构造阶段）构造的 handshake 帧暂存入独立 handshake deque（与 runtime queue **物理分离**），消费端逐帧消费过程中保留；**不与 live/pending 双计**（handshake 不代表立即释放——帧已在 deque 中，后续正常消费释放）。
  - 单 part 上限 `TOKEN_PART_MAX_BYTES=1MiB`；全局活跃 part 数 `TOKEN_LIVE_PARTS_MAX=32`。
  - **worst-case**：`8 × (512KiB queue + 8MiB handshake) + 4MiB live + 4MiB pending = 76MiB`（handshake buffer 为新增项；runtime 正常态无 handshake 占用的峰值仅 `8 × 512KiB + 4MiB + 4MiB = 12MiB`）。
...
- **handshake buffer overflow** → 503 `{"code":"sse_token_handshake_overflow","limit":8,"current":N,"bufferBytes":8388608}` + `Retry-After:5`。触发条件：handshake deque items 超 `TOKEN_HANDSHAKE_ITEMS=2048` 或 bytes 超 `TOKEN_HANDSHAKE_BUFFER_BYTES=8MiB`。与 `sse_token_subscriber_limit` 的关键区别：后者是**订阅者容量**溢出（同时 stream 数超 `token_stream_max_subscribers=8`），前者是**单个握手过程**的帧缓冲超限（同一条 stream 握手期间构造的 snapshot 帧集过大）。
...
- 503 `sse_token_handshake_overflow`（**v0.6 🆕**，token stream handshake buffer overflow；带 `{"limit":8,"current":N,"bufferBytes":8388608}` + `Retry-After:5`；触发于单次 handshake items 超 `TOKEN_HANDSHAKE_ITEMS=2048` 或 bytes 超 `TOKEN_HANDSHAKE_BUFFER_BYTES=8MiB`。与 `sse_token_subscriber_limit` 的区别：后者为订阅者容量溢出，前者为单个握手帧集过大，见 §6.x）
