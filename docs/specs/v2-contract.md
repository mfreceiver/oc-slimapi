...
> |---|---|---|---|---|
> | **2026-08-15** | **2（不变）** | **v2+additive** | **L1–L3 slim 整合四能力（全部加性，未 bump `X-Slimapi-Version`，仍 2）**：(A) `GET /slimapi/events?tokens=1` 可选下发 lean `token` 帧（仅字面 `"1"` 合法，其他→400 `invalid_tokens`，缺省零帧）；(B) `GET /slimapi/permissions` 跨目录 pending permission 聚合端点（questions 同款 envelope）；(C) `GET /slimapi/messages/{sid}?mode=merged` 服务端合并（placeholder 内联 full 投影，未知 mode 静默忽略）；(D) `/full/{mid}` single-flight 共享抓取 + transform_busy 预算吸收。health `features` 新增四 flag：`tokenCoalesce`/`permissionEvents`/`serverMerge`/`transformAbsorb`。详见 §2 / §3 / §1。 | §1 / §2 / §3 |
...
- **health `features` 清单（2026-08-15 加性，未 bump `X-Slimapi-Version`，仍 2）**：`/slimapi/health` 根级 `features` 新增四 flag——`tokenCoalesce`（A：`/slimapi/events?tokens=1` 可选 lean `token` 帧）、`permissionEvents`（B：`/slimapi/permissions` 跨目录聚合端点 + events 直推）、`serverMerge`（C：`/slimapi/messages/{sid}?mode=merged` 服务端合并）、`transformAbsorb`（D：/full 单条 single-flight + transform_busy 预算吸收）。四 flag 静态 all-true（同 release train，非渐进 flag），与既有 `tokenStream` / `thresholdedSkeleton` 并列（实现见 `features.py` / `routes/health.py`）。客户端能力探测：缺任一 flag → 该能力走既有回退（零回归）。
...
| GET | `/slimapi/sessions/{sid}/todo` | A | 🔒 | **todo thin 路由（加性新增 — T17/traffic Batch 3，未 bump `X-Slimapi-Version`，仍 2）**。透传上游 `GET /session/{sid}/todo`，近恒等投影（上游 `Todo.Info` `{content,status,priority}` schema 已最小——无白名单杠杆，路由价值 = gzip + cap + admission + 结构化错误）；`directory` 可选（仅 `X-Opencode-Directory` header 转发）。转换池 admission 先于 upstream GET + 流式 `read_with_cap`（超 `max_response_bytes`→413 `response_too_large`）+ worker gzip（**空 `[]` 跳过 gzip**——<64B 受益门）；**无 ETag/304**（本路由未接 Batch 2 ETag：响应无 `ETag` 头，任何 `If-None-Match` 被忽略、恒 200）；200 `Vary: Accept-Encoding, X-Opencode-Directory` + `Cache-Control: no-store`。上游 404→404 `session_not_found`（带 `sessionID`）；其他 4xx→502 `upstream_http_N`；5xx/网络/坏 JSON/非 list/逐项非 dict→503 `upstream_unavailable`；池满→503 `transform_busy`+`Retry-After:2`。**加性**：旧 sidecar 无此路由→catch-all 404 `thin_route_not_found`，客户端回退透传 `GET /session/{sid}/todo` |
| GET | `/slimapi/sessions/{sid}/children` | A | 🔒 | **children thin 路由（加性回归 — T17/traffic Batch 3，无状态重加，未 bump `X-Slimapi-Version`，仍 2）**。透传上游 `GET /session/{sid}/children`，每项经既有 `skeleton_session()` 投影（与 `GET /slimapi/sessions` 列表**逐字相同** keep/drop：丢 `cost`/`tokens`/`location`/`subpath`）。**状态量护栏（设计稿 §6.2）**：无 `X-Children-Version` 头、无 `childrenVersion` digest 字段、无 `childrenIDs[]`/`childrenComplete` list hints、无 cache/single-flight/SSE 失效——纯读（v1 状态机保持删除）。`directory` 可选转发。admission 先于 GET + cap-read（超限→413 `response_too_large`）+ worker gzip（**空 `[]` 跳过 gzip**）；**无 ETag/304**（`If-None-Match` 忽略、恒 200）；200 `Vary: Accept-Encoding, X-Opencode-Directory` + `Cache-Control: no-store`。上游 404→404 `session_not_found`（带 `sessionID`）；其他 4xx→502 `upstream_http_N`；5xx/网络/坏 JSON/非 list/逐项非 dict→503 `upstream_unavailable`；池满→503 `transform_busy`+`Retry-After:2`。**加性**：旧 sidecar→catch-all 404 `thin_route_not_found`，客户端回退透传 `GET /session/{sid}/children` |
| GET | `/slimapi/sessions/{sid}/diff` | A | 🔒 | **diff thin 路由（加性新增 — T18，2026-08-16，未 bump `X-Slimapi-Version`，仍 2）**。透传上游 `GET /session/{sid}/diff`，近恒等投影（上游 `Snapshot.FileDiff` `{file?,patch?,additions,deletions,status?}` schema 已最小——`patch` 大字段保留，gzip 是省流杠杆）；`directory` 可选（仅 `X-Opencode-Directory` header 转发）；**`messageID` 可选同名 query 原样透传上游**（缺省不发——上游语义=返回 `[]`：`summary.ts:129-137` 缺省/未知消息/非 user 角色均答 200 `[]`，空结果为正常 body 非错误）。转换池 admission 先于 upstream GET + 流式 `read_with_cap`（超 `max_response_bytes`→413 `response_too_large`）+ worker gzip（**空 `[]` 跳过 gzip**——<64B 受益门）；**无 ETag/304**（本路由未接 Batch 2 ETag：响应无 `ETag` 头，任何 `If-None-Match` 被忽略、恒 200）；200 `Vary: Accept-Encoding, X-Opencode-Directory` + `Cache-Control: no-store`。上游 404→404 `session_not_found`（带 `sessionID`）；其他 4xx→502 `upstream_http_N`；5xx/网络/坏 JSON/非 list/逐项非 dict→503 `upstream_unavailable`；池满→503 `transform_busy`+`Retry-After:2`。**加性**：旧 sidecar 无此路由→catch-all 404 `thin_route_not_found`，客户端回退透传 `GET /session/{sid}/diff` |
...
| GET | `/slimapi/permissions` | A | 🔒 | **跨目录 pending permission 聚合（加性新增，未 bump `X-Slimapi-Version`，仍 2）**。无参数（sidecar 自发现目录）；无 body；可带 `Accept-Encoding`。供 ocdroid 冷启动/重连恢复 pending permission 卡片（v2 曾删聚合端点，此前只能轮询 catch-all `GET /permission`，仅见 `process.cwd()` 实例）。**两阶段 fan-out（镜像 `/slimapi/questions`，oracle §B-1 同款 envelope）**：(1) `GET /experimental/session?roots=true&archived=true` 发现 distinct directory 集合（任一失败→整体 503 `upstream_unavailable`，无 envelope）；(2) 对每个 distinct directory 并发 `GET /permission`（带 `X-Opencode-Directory: <dir>`）。上游返**裸数组** `PermissionV1.Request[]`（字段 `id`/`sessionID`/`permission`/`patterns`/`metadata`/`always`/`tool?`）；sidecar 白名单投影（保留 7 字段，丢弃未知）+ 逐条 stamp `directory`。每 dir **独立隔离**：网络/5xx/非 list→`errors[]` code `upstream_unavailable`；4xx（含 unlikely 404）→`errors[]` code `upstream_http_N`；单 dir 失败不中断整体。内部预算三 knob（`permissions_max_response_bytes`/`permissions_fanout`/`permissions_max_aggregate_bytes`，ops 面）触发→`truncated:true` + `authoritativeDirectories` 降级。返回 envelope `{items, errors, authoritativeDirectories, discoveryComplete}`（语义见 §2「`/slimapi/questions` envelope」）。pending permission 应答仍走 catch-all + `X-Opencode-Directory`（§2 写路径）。健康公告 `features.permissionEvents`。 |
...
| GET | `/slimapi/messages/{sid}` | A | 🔒 | **骨架分页**（`?limit/before/mode/directory`）；**恒返回 skeleton 投影**——`?mode=full` 被静默忽略（不报错，仅返回 skeleton）；`?mode=merged`（**L2-CD-2，加性，未 bump `X-Slimapi-Version`，仍 2**）时页内 `thin_placeholder` 折叠消息由服务端按预算 fan-out 抓取 /full 并内联展开（剥 LSP diagnostics，与 `/full` 同一投影；其余消息与 `X-Next-Cursor` 与缺省逐字节一致；`mode=full` 及任何非字面 `merged` 值静默忽略，与缺省一致、不 400）；列表按 `time.created` **升序**；200 响应下发 **`X-Next-Cursor`** 头（opaque base64url 字符串，解析自 upstream `Link: <...?before=CURSOR>; rel="next"`），客户端用 `?before=<X-Next-Cursor>` 翻页向旧方向 drain；每条消息含加性 `contentFingerprint` 字段（终态语义/跨模式约束见 §2「消息内容指纹」节，2026-08-16 加性） |
...
| GET | `/slimapi/events` | A | 🔒 | 实例级策展 SSE（见 §3；`?tokens=1` 可选下发 lean `token` 帧——仅字面 `"1"` 合法，其他→400 `invalid_tokens`，缺省零帧、行为不变） |
...
- ~~`GET /slimapi/sessions/{sid}/children`~~ —— children 投影端点删除（含 `X-Children-Version` 头、`childrenIDs[]` / `childrenComplete` list hint 全部移除）。**2026-08-16 加性回归**（T17/traffic Batch 3）：`GET /slimapi/sessions/{sid}/children` 已**无状态重加**为 thin 路由（每项 `skeleton_session()` 投影，与 sessions 列表同 keep/drop；v1 状态量——`X-Children-Version`/`childrenVersion`/list hints——**保持删除**）；`X-Slimapi-Version` 仍 `2`，未 bump，加性。见 §2 端点表。同期加性新增 `GET /slimapi/sessions/{sid}/todo`（todo thin 路由，此前从未存在于 slimapi）。
...
- **`authoritativeDirectories`**（客户端契约依赖）：
  ...
  - **目录数组**（仅成功 directory，first-seen 保序）当 `errors` 非空 **或** `discoveryComplete == false`（发现截断）→ **partial**：客户端**仅可**对数组中所列 directory 做 replace（丢弃这些 directory 中不在 `items` 的旧 question）；**不得**对未列出的 directory 做 replace（它们的既有 pending question 必须保留——这些是 per-dir `/question` 失败或未发现的 directory，sidecar 未取得其当前 pending 集合，错误地 replace-all 会丢失它们的 pending question → **数据丢失**）。
- **`discoveryComplete`**（加性诊断，客户端可忽略 if absent-aware）：`true` 除非 `GET /experimental/session?roots=true&archived=true` 返回的 session 数正好填满 `_DISCOVERY_LIMIT`(=10000)（可能截断，降级 partial）。`roots=true` 只返顶层 session（数量 ≈ distinct workdir 数），实际恒 `true`。
- **`truncated`**（加性诊断，2026-08-08，客户端可忽略 if absent-aware）：`true` 当聚合 `items` 数超 `_MAX_AGGREGATE_ITEMS`(=10000) 时——后续 directory 不再 extend，`authoritativeDirectories` 同步降级为 succeeded list（partial-replace，与 discovery 截断相同语义），客户端不会因聚合截断丢弃未跳过 directory 的 pending question。缺省 `false`（absent-aware 客户端视为未出现）。

**边界**：
- 无任何 session（`/experimental/session` 返 `[]`）→ envelope `{items:[], errors:[], authoritativeDirectories:null, discoveryComplete:true}`（**权威空**：无 workdir 即无 pending question；replace-all 安全）。
...
### ETag / 304 条件请求（加性，2026-08-16；未 bump `X-Slimapi-Version`，仍 2）

适用于 4 个 GET 端点：`/slimapi/sessions`（列表）、`/slimapi/messages/{sid}`（列表，`mode=merged` 同样适用——以最终 splice 后 body 为准）、`/slimapi/agent`、`/slimapi/command`。客户端可选接入：不带 `If-None-Match` 时行为与旧版逐字节一致（仅多一个 `ETag` 响应头）。

- **验证器**：按实际 coding 派生——identity 编码 → 强 ETag `"<sha256(REP_VERSION ∥ \"identity\" ∥ identity_body)>"`；gzip 编码 → 弱 ETag `W/"<sha256(REP_VERSION ∥ \"gzip\" ∥ identity_body)>"`（canonical 输入恒为 identity body 字节 + coding id；全量 hex 不截断）。`REP_VERSION` = skeleton 投影版本 + 影响表示的 config 指纹——投影/配置变化 → 全部验证器轮换 → 构造上不可能误 304。两 coding 验证器互不通用（交叉 → 保守 200）。
- **304 判定**：`If-None-Match` RFC 9110 **弱比较**（忽略 `W/` 前缀按 opaque tag 比较）+ `*`。**管线照常执行不短路**——上游 GET/投影/admission 全部照跑（上游去重照常摊薄），命中仅省**下行传输体**（gzip 命中 = canonical hash 后零压缩零传输）。
- **304 头集合**：`ETag`（与 200 同值）+ `Vary`（与 200 完全一致）+ `Cache-Control: no-store` + 路由辅助头复制（messages `X-Next-Cursor`、sessions `X-Complete`——值来自**本次**管线计算）；无 body。
- **Vary 合并**：上述 4 端点 200/304 的 `Vary` 由 `Accept-Encoding` 合并为 `Accept-Encoding, X-Opencode-Directory`（directory 影响上游 workspace 路由，catalog/list 投影随 directory 变化）。
- **边界**：4xx/5xx 不带 `ETag`、不参与 304；HEAD 维持透传。ops 开关 `OC_SLIMAPI_ETAG_ENABLED=false` → 不输出 `ETag`、不判 304、`Vary` 不合并（逐字节等价开关关闭前行为）。

### 消息内容指纹 `contentFingerprint`（加性，2026-08-16；未 bump `X-Slimapi-Version`，仍 2）

`GET /slimapi/messages/{sid}`（缺省与 `mode=merged`）每条消息 skeleton 加性字段 `contentFingerprint: string`，格式 `"<vN>:<sha256hex>"`（全量 hex 不截断；`vN` 为指纹方案版本前缀）。

- **生成位置**：缺省列表 = skeleton 投影完成时；`mode=merged` = placeholder 内联 splice 完成后**重算覆盖**（反映最终 full 内容；full 抓取失败/预算不足/坏响应等降级路径**不重算**，保留 skeleton 期指纹）。
- **规范化**：输入 = 排除 `contentFingerprint` 自身后的消息投影 dict，`sort_keys` 序列化（parts 保持上游序、无数值规范化）后 SHA-256。指纹只覆盖 sidecar 投影后的表示，不含被丢弃字段。
- **终态语义**：①同输入（同投影、同规范化、同 `vN`）→ 恒同指纹（确定性，跨进程重启成立）；②**指纹不同 ⟹ 输入不同**（单向蕴含，必然）；③指纹相同 → 输入相同（以 SHA-256 碰撞可忽略为前提）。**不提供单调性**：指纹不是序号/revision，不承诺随内容变化单调递增，客户端不得据此排序或版本比较。
- **跨表示模式不可比较**：缺省与 `mode=merged` 对同一消息产生**不同**指纹（merged splice 后输入包含 full 内容）——`vN` 前缀**不区分**模式，客户端**仅在同一表示模式内**比较指纹。
- **与 digest/watermark 关系**：指纹是 `(updatedAt, messageId)` 双水印去重推进的**补充证据**（内容级等价性判定），不替代、不改变 digest 帧或水印推进规则；digest 帧不带指纹。
- **开关**：`OC_SLIMAPI_MESSAGE_FINGERPRINT_ENABLED=false` → 不输出该字段（逐字节等价开关关闭前行为）；该开关状态参与 `REP_VERSION`（关闭 → 全部 ETag 验证器轮换）。
...
- 帧：
  ...
  - **lean `token` 帧（L2-A，`/slimapi/events?tokens=1` 时下发；加性，未 bump `X-Slimapi-Version`，仍 2）**：`{type:"token", sessionID, messageID, partID, delta}`——载荷为 token-stream flush loop（100ms / 4KiB 窗口）对每个 `(sessionID, messageID, partID)` 完成窗口拼接的**增量 concat**（覆盖所有 session 的合并 delta，MVP 无 per-sid 过滤）。**无 SSE `event:` 头**（客户端按 `data.type=="token"` 分发，镜像 raw 直推风格）；**无 `partEventRevision`、无 `directory`**（oracle §A-2：directory 单用户下冗余；权威 part-revision / full-text 追踪仍在 per-session stream 与 `/messages`）。**帧语义声明**：`token` 帧是**动画层 only**——丢帧由 digest + `/messages` 兜底（客户端不得以 token 帧为权威全文/修订来源）；无 replay 承诺。**迁移期互斥**：events-token 与 per-session token stream 在**客户端**互斥（服务端不强制踢双订阅者，双份投递风险由客户端规避）。**背压**：复用既有 `Subscriber.put` T3 守卫——溢出与 per-session token stream 同语义（`resync{reason:"subscriber_backpressure"}` + 断连）。**生命周期**：events-token 消费者计入 token flush loop 合并账本（双账本）——即使无任何 per-session token stream 打开，仅 events-token 消费者也能保活 flush loop（~100ms 节奏持续收帧）；最后一个连接断开（两账本皆空）flush loop 停止。
...
- **`session_deleted` 服务端终止（2026-08-08，加性 wire 修正）**：上游 `session.deleted` 事件到达时，sidecar 对该 sid 的所有 token 订阅者**同步**发 `resync{session_deleted,sessionID}` 后立即终止连接（发 STOP 关闭流）。客户端收到该 resync + 连接关闭后应视为该 session 的 token stream 已结束；如需重建须重新订阅。此前的 deferred-resync-only 行为（仅 fanout resync、不主动断开、依赖客户端关连接）已废弃（资源泄漏修复：避免 deleted session 的订阅槽位/连接/flush loop 泄漏）。
...
- **v2 删除**：v1 的 `GET /slimapi/projects`（步骤 1 可选显式刷新 allowlist）与 `GET /slimapi/permissions`（步骤 3）已从冷启动顺序中移除——这些端点在 v2 中不存在。客户端如需 q/p 快照，通过 SSE 订阅后实时推流获取（§3 的 6 个 q/p 帧在连接建立后仍直推）；**SSE 无 backlog replay**——订阅之前已到达的 pending q/p 不会重放。（**2026-08-15 加性回归**：现提供 `GET /slimapi/permissions` 跨目录 pending permission 聚合端点，见 §2——冷启动/重连后补拉 pending permission 用本端点，替代对 catch-all `GET /permission` 的轮询。）
- **pending question 跨目录冷启动补拉（2026-08-05 加性）**：`GET /slimapi/questions`（§2）已加性回归为**跨目录聚合端点**，用于冷启动/SSE 重连后补拉**所有 directory** 的 pending question（每条带 `directory` 字段）。客户端冷启动顺序可在步骤 2 后加：`GET /slimapi/questions` → 按 envelope 语义（§2「`/slimapi/questions` envelope」）做 replace-all（`authoritativeDirectories==null`）或 partial replace。该端点修复 slim-mode 冷启动看不到 `workdir ≠ process.cwd()` 目录 pending question 的 bug（上游 `GET /question` per-Location；sidecar fan-out 各 dir 合并）。permission 无等价聚合端点。（**2026-08-15 加性**：`GET /slimapi/permissions` 已加性新增为等价聚合端点，见 §2 端点表；permission 与 question 同款两阶段 fan-out + envelope。）
...
- **服务端合并 `mode=merged`（L2-CD-2，加性，未 bump `X-Slimapi-Version`，仍 2）**：`GET /slimapi/messages/{sid}?mode=merged` 在骨架列表基础上，将页内被折叠为 `thin_placeholder_` 的 placeholder part 由服务端按预算 fan-out 抓取 `/full` 并**内联展开**——placeholder parts 替换为该消息 full 投影（剥 LSP diagnostics，与 `/full` 同一投影，oracle §C-2），`omitted:["parts"]`/`hasFull` 状态随之清除；**其余消息（含 placeholder 未展开项）与 `X-Next-Cursor` 与缺省逐字节一致**。预算：`merged_fanout`（并发，默认 8）/`merged_max_fulls_per_page`（每页条数，默认 16）/`merged_max_bytes`（字节，默认 8 MiB，**预留/退款**记账——每项读取上限=min(单消息上限,剩余预算)，预算耗尽的未开始项不发起抓取）。单项失败/截断/超预算仅该项**渐进降级**保持 placeholder 原样（不 500 整页、不 413、不报错）。fan-out 经 `singleflight.fulls` 与并发 direct `/full` **同键去重**（键含 scope+directory，上游仅 1 次 GET）；fan-out **不持** transform 池槽（默认 `max_transforms=1` 下 per-full 持槽会把 16 个网络 GET 串行化并饿死并发 transform/direct /full），最终单次 offload 打包（争用沿用既有 `transform_busy` 503 形状）。splice 后响应内联总量 ≤ `merged_max_bytes`。**`mode=full` 及任何非字面 `merged` 值静默忽略**（与缺省逐字节一致，不 400，oracle §C-1）。
- **`/full/{mid}` single-flight + transform_busy 预算吸收（L2-CD-1，加性，未 bump `X-Slimapi-Version`，仍 2）**：(1) **single-flight**——并发同 `(sid, mid, directory)` 的 /full 请求（及 merged fan-out 抓取）合并为**一次**上游 GET（per-key in-flight 去重 + 完成后 ≤1s join 窗口；transform 仍各自做）。可观测差异：同键 /full 在 1s 内重复请求可能复用共享抓取结果（body 与首次抓取一致，不重复打上游；上游计数/access log `upIn` 只记一次）。(2) **预算吸收**——/full 的 transform 槽等待不再单次 `transform_wait_seconds`（默认 2s）即 503，而是在总预算 `OC_SLIMAPI_TRANSFORM_ABSORB_BUDGET_SECONDS`（默认 2.5s，T0 引入）内按剩余预算收窄重试（oracle §D-1：最坏累计等待不超预算）：瞬时占用（>2s 且 <2.5s）现返回 **200**（原先 503）；占用超预算才 503，**503 形状逐字节不变**（`{"code":"transform_busy","retry_after":2}` + `Retry-After: 2`）。503 时上游请求照旧不发出（admission 先于 GET）。
...
- 400 `version_required` / `version_incompatible` / `directory_not_allowed` / `invalid_directory_count`（**v2 无独立生产路径**——q/p 聚合路由删除后此码无触发路径；保留作结构性守卫文档参考，实现中无对应 wire 输出）
  - **`invalid_tokens`（加性新增，2026-08-15，未 bump `X-Slimapi-Version`）**：`GET /slimapi/events` 的 `tokens` query 参数**仅字面 `"1"` 合法**；其他任何值（含 `0`/`true`/空串/非 `1` 的任意值）→ 400 `invalid_tokens`。缺省（无参数）合法，行为逐字节不变。
...
- **可观测性（v2 保留，加性，不 bump、不构成协议契约依赖）**：每请求由最外层中间件生成/透传 `X-Request-ID`（请求头 + 响应头回显，并透传上游 opencode；入站值**限可打印 ASCII（0x20–0x7e）**——含 CR/LF/控制字符/空白/非 ASCII（多字节 Unicode）/超 128 字节则丢弃改生成新 uuid，2026-08-08 收紧）；access log 每条记录含 `requestId` 字段。客户端可 echo 该头做跨 sidecar↔opencode 关联诊断。
...
> 3. **普通 JSON / catalog 响应**（sessions / messages / command / agent / health / ready / metrics / questions / directories / actions）按内容协商（`Accept-Encoding`），`Vary: Accept-Encoding`（agent / command / sessions / messages 列表自 2026-08-16 Batch 2 起合并 `X-Opencode-Directory`）。压缩策略**按路由分两族**（终审 B2 对账，2026-08-16）：
>    - **无条件压缩**（accepts gzip 即压，无受益门）：`agent` / `command`（catalog 族）与 `sessions`（列表 + status）——历史基线行为；Batch 2 ETag 的 coding 预测依赖其确定性（gzip 表示可预测），**sessions 与 v2 基线一致，刻意不引入受益门**。`health` / `ready` / `metrics` / `actions` 等控制面小响应同走 `json_response`，属同族。
>    - **受益门**（两档机制，64B 阈值）：① `todo` / `children`（T17）、`diff`（T18）——**阈值门**：≥64B 且接受 gzip 即无条件压缩（无压缩后对比；`_catalog_common.py` worker 路径）；② `messages` 列表、`questions` / `permissions` envelope、`directories`——**真受益比较**（`compress_if_beneficial`，压缩后更大则回退 identity）。
...
- **v2 删除**：v1 §12 的 `/sessions/{sid}/status`、`/questions` + `/permissions`（repeated 1-32 + null 聚合）、POST q/p reply/reject/permission（directory 来自 routeToken）行已移除——这些端点在 v2 中不存在。（`GET /slimapi/sessions/status` 于 2026-08-03 加性回归，见 §2；v1 的 per-session `/sessions/{sid}/status` 仍删除。`GET /slimapi/permissions` 于 2026-08-15 加性回归为跨目录聚合端点，见 §2——v1 的 repeated-1-32/null 聚合**不复用**，现为 questions 同款 fan-out envelope。）
