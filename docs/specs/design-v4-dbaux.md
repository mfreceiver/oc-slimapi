...
6. **P99 熔断样本口径（冻结）**：
   ...
   - **恢复探针（4.12.1 FIX-CORR-3，两段式）**：熔断后进入半开（half-open），周期（30s）单次探针：`SELECT 1` 仅作连接存活预检（延迟不计时、不参与判据）；关断判据 = 路径同构 canary（`session LEFT JOIN project`——SELECT 需引用 p 列否则 SQLite 消除 join——按 `time_updated DESC, id DESC` 全扫+排序路径，mode=ro 纯 SELECT）的延迟。**连续 K=3 个好探针 → 转入 probation 试用期（非直接闭合）**；任一慢探针或探针异常清零连击；
   - **probation 试用期（4.12.1 FIX-CORR-3r3）**：真实查询恢复放行、逐查询以延迟标量判定（判据不读样本窗口）：任一 ≥20ms 慢真实查询**立即复 trip**（豁免最小样本门——与探针间隔/样本窗口时序解耦；canary 残余便宜度所致误闭合的最坏暴露面**限 probation 误恢复期间** = 1 个慢真实查询，复 trip 后即恢复逐查询判定——正式 graduation 回到 closed 后不再承诺绝对一查询上限，恢复常规 P99/最小样本判定；r4 起已排队请求在**出队执行前**二次检查熔断器，open 即走与入口拒绝一致的降级（不执行 SQL）——暴露面不含队列深度）；连续 N=3 个好真实查询才正式闭合。探针样本不进 P99 窗口；trip 不清空窗口样本（closed 态画像连续性）；
   - **hysteresis**：恢复阈值 < 熔断阈值（恢复探针 <10ms 才计好，防抖——避免 20ms 临界抖动反复开关）；
...
- 护栏 = 每查询延迟采样（§2.7 口径：60s 滑窗 + ≥10 样本 + warmup 豁免）P99 ≥ 20ms → 熔断 → 全降级矩阵（§7）+ 告警；恢复走半开探针（两段式 canary + 连击迟滞）+ probation 试用期（§2.3-6）。
