...
| `TextPart.text` | ~~UTF-8 编码字节 > 阈值 → 省略为 `null` + expandRefs；否则完整保留~~ **[3.2.0] 已废止：永远全量内联，不折叠**（owner 决策 2026-08-17，对话正文不缩减；wire 见 v4-contract.md §10.2） | ~~`text_inline_max_bytes=2048`~~ 无 |
