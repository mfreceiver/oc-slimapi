...
- `app.py` 先按 `health → sessions → messages → events → metrics → token_stream` 注册 `/slimapi/**`（`token_stream` 在 catch-all 之前以确保不被 `/{path}` 吞掉），最后调用 `install_proxy(app)` 注册 catch-all，故 thin route 不会被反代吞掉。
