...
- `app.py` 先按 `health → agent → command → sessions → messages → events → metrics → token_stream` 注册 `/slimapi/**`（`token_stream` 在 catch-all 之前以确保不被 `/{path}` 吞掉），最后调用 `install_proxy(app)` 注册 catch-all，故 thin route 不会被反代吞掉。
...
- **access log 文件发现规则（2026-07-29 加性，ops 面）**：由旧 `logs/access.jsonl`（`RotatingFileHandler` 10MiB×5）改为**按天切分** `access-YYYY-MM-DD.jsonl`。启动压缩**早于今天（`< today`）**的 `.jsonl` → `.gz`（原子 `.gz.tmp`+rename）+ 迁移**当前 `access_log_dir` 内**无日期文件为 `access-legacy-*.jsonl.gz`（**不跨目录迁移**；生产从旧相对目录升到 `StateDirectory` 时旧位置历史日志需运维手动移动）；后台 maintenance loop（默认 1h）周期 compress+prune（`OC_SLIMAPI_ACCESS_LOG_RETAIN_DAYS`，默认 `0`=不删；prune 严格匹配 `access-YYYY-MM-DD.jsonl(.gz)`，**不含** `access-legacy-*.jsonl.gz`——后者永久保留）。生产 systemd unit 覆盖路径到 `StateDirectory`（`%S/oc-slimapi/logs`）；本地开发默认相对 `logs/`。旧 env `OC_SLIMAPI_ACCESS_LOG_PATH`/`_MAX_BYTES`/`_BACKUPS` 标 deprecated（保留兼容）。
