...
- **测试**：协议矩阵用例表（重放/缺口/过期/重启 epoch/背压/**tokens=1 → 400**（已裁决）/ID 无倒退断言/上游断连 barrier/组合输入优先级，17 条 REPLAY 用例见 design-v4-sse-replay §4），落地于 B3b
