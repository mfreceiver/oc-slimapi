...
|---|---|---|---|---|---|
| **GET `/slimapi/sessions`**<br>无应用层鉴权；可带 `Accept-Encoding`。参数：`directory:str?`、`roots:bool=false`、`limit:int=100`（1–1000）、`start:int?`（≥0）、`search:str?`；无 body。 | `GET http://127.0.0.1:4096/session`；query 始终有 `limit`、`roots`，其余非空才传。directory 存在时同时传 `?directory=` 和 `X-Opencode-Directory`。 | `Session[]` 裸数组；通常 200；上游也可能 4xx/5xx。 | directory 经 `normalize_directory()`（[Unreleased] 不 gate）；每项调用 `skeleton_session()`，只留 session 基础字段、`time{created,updated,archived}`、`summary{additions,deletions,files}`、`revert{messageID,partID}`。 | 200：`Session[]` 裸数组；支持 gzip，`Vary:Accept-Encoding`。上游 4xx→502 `upstream_http_N`；5xx/网络→503 `upstream_unavailable`；200+坏 JSON/坏 shape→503 `upstream_unavailable`（body 均 `{"code":...}`，v0.2.1 对齐 §7）。业务 allowlist gate 已移除（[Unreleased]）；FastAPI 参数错误 422。 | 代码不接收 cursor；`limit=0` 在 sidecar 被校验拒绝，避免 legacy 把 0 解释为全量。**v0.2.1：失败路径对齐 §7 coded**（4xx→502 `upstream_http_N` / 5xx·网络→503 `upstream_unavailable` / 200+坏 JSON·坏 shape→503）；原静默透传 upstream body、网络错→FastAPI 默认 500 已修。
...
实现：对 upstream 用 `httpx.HTTPStatusError` 精确判 404；其它 4xx → 502；仅网络/5xx/解析失败 → 503。批量 `GET /slimapi/sessions/status`（透传 upstream status + map）同样不 gate allowlist（[Unreleased] 对齐）。
...
`/slimapi/messages/{sid}`、`/slimapi/messages/{sid}/since/{ts}`、`/slimapi/messages/{sid}/full/{mid}`、**`/slimapi/messages/{sid}/full?ids=`（G6）** 四条消息路由统一经 `_resolve_messages_directory` 处理 directory。**[Unreleased] allowlist gate 已移除**：directory 不再因 ∉ allowlist 返 400，仅规范化后透传给上游 opencode。复用的 `normalize_directory()` 仍保留以保持转发一致：
