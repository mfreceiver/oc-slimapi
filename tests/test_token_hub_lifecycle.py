...
class TestBoundedDisabled:
    def test_cap_evicts_oldest(self, monkeypatch):
        ...
        monkeypatch.setattr("oc_slimapi.sse.token_hub.TOKEN_DISABLED_MAX", 3)
        # Effectively disable TTL so cap is the binding constraint.
        monkeypatch.setattr("oc_slimapi.sse.token_hub.TOKEN_DISABLED_TTL_MS", 10**9)
        th = TokenStreamHub()
        for i in range(4):
    ...
    def test_ttl_expires_old_entries_on_next_insert(self, monkeypatch):
        ...
        monkeypatch.setattr("oc_slimapi.sse.token_hub.TOKEN_DISABLED_MAX", 100)
        monkeypatch.setattr("oc_slimapi.sse.token_hub.TOKEN_DISABLED_TTL_MS", 100)
        th = TokenStreamHub()
