...
- **SSE `id:` 序列与 Last-Event-ID 重放（B3b，v4-only——v3 SSE 帧名帧形零变化）**：
  ...
  - **v4 订阅握手不预发连接私有帧（rev-gate BLOCKER-1 裁决）**：v4 下两流订阅均**抑制 `server.connected`**（不在冻结无-id 控制帧集合 meta/resync/heartbeat 内）；token 流 v4 握手**不预发**历史 `message.removed` tombstone（重连由重放日志带 `id:` **恰一次**回放——预发会造成无 id 双发）与 live part `message.part.snapshot`（服务端 v4 永不发 snapshot 帧，状态对齐走 HTTP）；内存逐出再锚定的 per-sub snapshot 直发同样对 v4 抑制。v4 首连 = meta + 后续 live 帧；v4 带游标重连 = meta + replay（tombstone 带 ID 恰一次）+ live。v3 握手时序（`server.connected` → tombstone 预发 → flush → snapshot → 入扇出）**逐字节不变**。
  - **v4 非冻结 reason 路径 = 连接终结（rev-gate R3 裁决）**：v4 `resync.reason` 值域冻结为上述四值；五条 v3 legacy reason 路径——全局/token 流背压溢出（`subscriber_backpressure`）、token 内存逐出（`token_memory_limit`）、会话空闲（`session_idle`）、会话删除（`session_deleted`）——在 v4 下**不发 resync 帧直接终结连接**（断连本身即服务端可观察信号）。恢复路径：`Last-Event-ID` 重连 → 窗口内 replay / 窗口外冻结 reason resync → 需全量状态时 HTTP 对齐；`session_deleted` 的删除语义由全局 `session.digest` 控制面表达。v3 各路径原 resync 帧与帧序**逐字节不变**。
...
- **directory allowlist fail-closed（B4-4，契约 §5.7a，部署配置面）**：新 env `OC_SLIMAPI_DIRECTORY_ALLOWLIST` 三态——**未配置**（缺省）= 机制禁用，现状逐字节零变化；**显式空串** = `/slimapi/file/**` 三端点全部 403 `{"code":"directory_not_allowed"}`（不泄露目录存在性）+ 启动 warning；**非空**（冒号分隔路径清单）= `/slimapi/file/**` 仅白名单目录子树放行 + GlobalHub SSE 帧过滤（digest 与 q/p IMMEDIATE 帧中 directory 非白名单/无法判定 → 丢弃+计数，放行帧字节不变）。**canonical 匹配（realpath）**：条目与候选 directory 均经 realpath canonical 化后匹配（防 `..`/尾斜杠/**symlink** 绕过）；启用时**相对 directory**按无法判定处理（403/丢帧）；`/file/**` 授权通过后向**上游转发 canonical directory**（realpath 形式——授权与实际访问对象绑定；未配置时原值逐字转发）；根 symlink retarget 后须重应用配置或重启方可生效（配置点失效重解析）。`/slimapi/health` features 增 `"allowlist":{"enabled":<bool>[,"droppedEvents":<int>]}`（只报布值不泄露清单）。**启用属部署事项，客户端无必改点**。
- **B1b 阶段1：q/p sweep shadow 观测（内部机制，wire 无新路由）**：每目录 30min±20% jitter 独立错峰纯模拟调度（cold-set 判定 + 日预算闸），**零真实 sweep 上游请求**（shadow 零逃逸）；观测经 `GET /slimapi/metrics` 新增 `sweep` 节暴露（`triggers_total`/`cold_hits`/`skips`/`budget_exhausted`/`est_bytes_total`/`known_directories`，启用时才有）；traffic bucket `sweep` 保留给阶段2 真实请求分类（阶段1 恒空）。为 3.3.0 后维护观测期（B1b 阶段2）提供数据。
...
- **PatchPart `files` 投影归一化（wire 形状变更——owner 批准例外，wire 版本维持 4 不 bump，随 4.9.0 minor 发布；权威 = `docs/specs/v4-contract.md` §10.2 修订四）**：`GET /slimapi/messages/{sid}` skeleton 中 patch part 的 `files` 由上游 `string[]` 原样保留（≤4.8.0 行为）改为**归一化对象数组**——源 string 条目 → `{"path": <s>}`；legacy object 条目 → 既有四键投影 `{path, additions, deletions, status}`；非 string/object 条目跳过。**cap 10 条**：有效映射条目超 10 → compact 列表截为前 10，并附 part 顶层整数 `filesTotal` = 源数组长度（**源计数口径**，含无效条目——非可展示文件数）；未超限不附。part 级 `diffStats` 维持防伪造守卫（int-only 校验：仅当源条目携带合法非负 int 计数才注入、非法值计 0、归一化 `{"path"}` 派生条目永不注入）。**owner 例外说明（D1-r 裁定 2026-08-21）**：本条为 wire 版本不变的破坏性 wire 形状变更，按发版规约「major 只跟 wire 协议大版本走；wire 不 bump 的破坏性变更发 minor」（`docs/release.md` §1.1/§1.3）随 **4.9.0 minor** 发布——依据 = 双客户端双形状兼容已实证（ocdroid `Part.kt` PartFilesSerializer / oc-webui `PartCards.vue` toFiles）+ 未知严格 `string[]` 消费方风险由 owner 知情接受；例外与必改点同步记录于 v4-contract §10.2 修订四修订头。**客户端必改点**：**严格按 `string[]` 解析 `files` 的消费方需改**（按对象数组/双形状解析）；ocdroid 与 oc-webui 已实证双形状兼容，**零必改**。

### Added

- **compress 折叠卡 title 合成（加性，wire 版本不 bump；v4-contract §10.2 修订四）**：tool part `tool=="compress"` 且投影 state 无 `title` 时合成标题——求值顺序（冻结）：`state.input` 为 object → `input.content` 非空数组 → `content[0]` 为 object → `title = clip(topic,160) ?? clip(summary,160) ?? "压缩 {N} 段"`（clip：仅 str、strip、空白串=缺失、按字符截断不附省略号）；任一前置不满足保持无 title（无兜底文案）。`content` 不进 input 投影白名单（`part_state_input_full` 展开路径零变化）。仅 skeleton 视图保证——`mode=merged` 成功 splice 后 title 回上游原状。
- **`state.output` 省略附 `outputBytes`（加性）**：tool state `output` 在场、非 null/空串、且因 4 KB/字段或 16 KB/消息预算省略时，state 附整数 `outputBytes` = 被省略字段值的 **JSON wire 字节数**（canonical 序列化长度，含引号/转义——非裸 UTF-8 文本长度）。`error` 省略不附对应键；不进 `omitted`/`hasFull`；merged/full 视图自然无此键。
- **tool `state.metadata` 增补 `files` compact 投影 + `filesTotal`（加性；apply_patch 等）**：metadata 投影键集增 `files`，专用 compact 映射（非 verbatim）：条目 → `{path: relativePath ?? filePath, additions?, deletions?, status?: type}`，剔除 `patch` diff 正文与 `filePath`/`relativePath`/`type` 原键；cap 10 + `metadata.filesTotal`（源计数口径，同 patch 侧）。compact 投影恒有损 → `part_state_metadata_full` ref 触发条件增补「源 `metadata.files` 非空」（判定基于源值——全畸形条目映射为空时 ref 仍在场）。
- **metadata aggregate `diffStats` 注入优先级链 + edit 合成注入（加性）**：tool metadata `diffStats`（thresholding 后注入、永不 omit）按冻结链判定：⓪ 源 `diffStats` 合法则**保留源值**（派生永不覆盖）→ ① `filediff` 结构化有效（异常安全化：非法值计 0、永不抛异常）→ ② `files` 有效 → ③ `tool=="edit"` 且 `metadata.diff` 可解析（单遍状态机：`Index:`/配对 `+++`/`---` 头、`@@` hunk 行数耗尽、`/dev/null` 删除/新增、孤立 `Index:` 整体放弃、`truncated==true` 跳过合成、无有效文件段不伪造）→ ④ 不注入。edit 同步注入 `metadata.files` 合成投影（`{path, additions, deletions}`，走同一 cap 10/filesTotal/ref 管线）；`state.metadata.diff` 本身维持省略。
- **expand `part_state_metadata_full` 对 edit 增补合成 `files`（加性；v4-contract §14.2 修订四）**：`GET /slimapi/messages/{sid}/expand/part_state_metadata_full/...` 对 `tool=="edit"` 且源 metadata 无 `files` 键且 `truncated` 非 true 且 diff 解析成功的 part，返回 metadata 附加 `"files"` = 完整解析列表（**无 cap**——skeleton cap 10 外的第 11+ 条经展开端点可达）；其余情形返回原始 metadata 去 `diagnostics`（既有行为）。
- **ETag 全量轮换（表示域版本 bump 的可观察效应，4.8.0 后第二次）**：skeleton 表示域 REP_VERSION `skeleton-v1` → `skeleton-v2`——同输入下新旧 v4 validator 必然不同，升级后旧 v4 ETag 全部自然失效重拉（预期内一次性流量；`contentFingerprint` 无 `vN` 改动，派生字段自然进入指纹）。
