...
- **B1b 阶段1：q/p sweep shadow 观测（内部机制，wire 无新路由）**：每目录 30min±20% jitter 独立错峰纯模拟调度（cold-set 判定 + 日预算闸），**零真实 sweep 上游请求**（shadow 零逃逸）；观测经 `GET /slimapi/metrics` 新增 `sweep` 节暴露（triggers/cold_hits/skips/budget_exhausted/est_bytes/known_directories，启用时才有）；traffic bucket `sweep` 保留给阶段2 真实请求分类（阶段1 恒空）。为 3.3.0 后维护观测期（B1b 阶段2）提供数据。
